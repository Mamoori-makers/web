...
export default function Roadmap() {
  return (
    <div>
      <p className="my-2 text-sm font-bold text-yellow-700">
        ...
        {ROADMAP_CONTENTS.map(({ title, description, link, image }) => {
          return (
            <Link
              href={link}
              key={title}
              className="my-2 inline-block rounded-md border-2 border-stone-200 p-5"
            >
              <Image src={image} alt={title} width={40} height={40} />
              <h2 className="my-2 text-base font-semibold">{title}</h2>
